...
describe("CalendarService#get", () => {
  ...
  beforeEach(() => {
    ...
    (global as any).CalendarApp = {
      getCalendarById: jest.fn().mockImplementation(id => ({
        get: (_start: Date, _end: Date) => eventsMap[id] || []
      })),
    };
  });
  ...
  it("parses clubPractice events correctly", () => {
    ...
    eventsMap[Config.Calendar.id.clubPractice] = [mockEvent(title, "ignored", new Date("2025-07-10T12:00:00"))];

    const result = service.get(EventType.ClubPractice, new Date("2025-07-01"), new Date("2025-07-31"));
    expect(result).toHaveLength(1);
    const ev = result[0];
    expect(ev.location).toBe(Config.PRACTICE_LOCATIONS["Jinja"]);
  ...
  it("parses externalPractice events correctly", () => {
    ...
    eventsMap[Config.Calendar.id.externalPractice] = [mockEvent(title, "外部場所", new Date("2025-07-11T14:00:00"))];

    const result = service.get(EventType.ExternalPractice, new Date(), new Date());
    expect(result).toHaveLength(1);
    const ev = result[0];
  ...
  it("parses match events with and without classes correctly", () => {
    ...
    expect(ev2.targetClasses).toEqual(StringUtils.formatStrictKarutaClass(""));
  });

  it("parses internalDeadline events correctly", () => {
    const title = `[EF]|締切イベント外部`;
    eventsMap[Config.Calendar.id.internalDeadline] = [mockEvent(title, "ignored", new Date("2025-07-14T00:00:00"))];

    const result = service.get(EventType.InternalDeadline, new Date(), new Date());
    expect(result).toHaveLength(1);
    const ev = result[0];
    expect(ev.targetClasses).toEqual(StringUtils.formatStrictKarutaClass("EF"));
