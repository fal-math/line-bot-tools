    ...
    if (text.trim().includes('外部練追加フォーマット')) {
      ...
      try {
        this.calendar.createCalenderEvent({
          ...
          summary: `外部${parsed.event.title}${parsed.event.timeRange} ${parsed.event.targetClasses}:${DateUtils.formatMD(parsed.deadline)}〆(参加ポチ)`,
      ...
      try {
        this.calendar.createCalenderEvent({
          ...
          summary: `〆${parsed.event.targetClasses}|外部${parsed.event.title}${parsed.event.date.toLocaleDateString()}`,
