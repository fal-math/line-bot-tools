export function setupTriggers_(): void {
  ...
  ScriptApp.newTrigger('announceDeadlineToday').timeBased().everyDays(1).atHour(9).create();

  /** 会〆当日通知: 毎日 21:00 */
  ScriptApp.newTrigger('notifyDeadlineToday').timeBased().everyDays(1).atHour(21).create();

  /** 本〆当日通知: 毎日 07:00 */
  ScriptApp.newTrigger('notifyFinalToday').timeBased().everyDays(1).atHour(7).create();
